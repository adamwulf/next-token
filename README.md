...
- **Sampling meta-parameters** (collapsible) — Temperature (0–2), Top-k (1–5), Top-p (0.1–1.0). Collapse the section to save space; a compact `temp · top-k · top-p` summary shows next to the title while collapsed. Dragging any slider reshapes the on-screen bars instantly, **client-side, with no server call** (the model's probabilities don't depend on these params — see below). Candidates ruled out by top-k / top-p are greyed out. Whatever the bars show is exactly what **Next token** samples from.
